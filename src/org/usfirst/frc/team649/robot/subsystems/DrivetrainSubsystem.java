...
//		compressCAN.setClosedLoopControl(true);
	...
	public void autoShift(){
		...
		double value = Math.abs(leftEncoder.getRate()) + Math.abs(rightEncoder.getRate());
		if (value < RobotMap.Drivetrain.MAX_LOW_SPEED*1.85 && value > RobotMap.Drivetrain.MAX_LOW_SPEED*1.8){
			
		}else if(value > RobotMap.Drivetrain.MAX_LOW_SPEED*1.75 && time.get()> 0.2){
			shiftState = true;		
			time.reset();
		}else if(value< RobotMap.Drivetrain.MAX_LOW_SPEED*1.75 && time.get() > 0.2){
			shiftState = false;
